...
export default function GapAnalitico() {
  ...
  const filtrados = useMemo(() => {
    ...
    if (filtroSt === 'Com Código' || filtroSt === 'Fat. Com Código')
      r = r.filter(x => x.cod && x.cod.trim() !== '' && !/PRO EMAG|NADH|PROT MESOTERAPIA|protocolo|gestrinona|testosterona|estradiol/i.test(x.proc ?? ''))
    if (filtroSt === 'Sem Código' || filtroSt === 'Fat. Sem Código')
      r = r.filter(x =>
        (!x.cod || x.cod.trim() === '' || /PRO EMAG|NADH|PROT MESOTERAPIA/i.test(x.proc ?? '')) &&
